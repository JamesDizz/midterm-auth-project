...
export default function Profile() {
  ...
  // Add two separate states for each password field
  const [showOldPassword, setShowOldPassword] = useState(false);
  const [showNewPassword, setShowNewPassword] = useState(false);

  useEffect(() => {
    // ... (useEffect for redirection)
    if (status === "authenticated" && session.user?.onboarded === 0) {
  ...
  if (status === "loading" || (status === "authenticated" && session.user?.onboarded === 0)) {
    // ... (loading state)
  ...
  return (
    <div className="relative min-h-screen bg-gray-900 text-white p-4 flex items-center justify-center">
      <button 
        ...
        className="absolute top-4 right-4 bg-red-600 text-white font-semibold py-2 px-5 rounded-xl hover:bg-red-700"
      >
        Sign Out
      </button>
